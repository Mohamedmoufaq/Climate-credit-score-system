...
# ============= DYNAMIC LOCATION API (Nominatim) =============
def get_location_from_nominatim(place_name):
    """
    Fetch latitude and longitude from OpenStreetMap Nominatim API
    Works for: State, City, Village, or any place name in India
    Returns: (lat, lon) or (None, None) if not found
    """
    try:
        url = f"https://nominatim.openstreetmap.org/search?format=json&q={place_name}%20India&limit=1"
        headers = {'User-Agent': 'Mozilla/5.0 (Climate-Credit-App)'}
        ...
        print(f"Nominatim API error for '{place_name}': {e}")
...
def get_or_create_profile(user):
    ...
    # Auto-assign default ROLE_OFFICER to new users
    if created and not profile.role:
        profile.role = UserProfile.ROLE_OFFICER
...
def dashboard(request):
    ...
    severe_flags = applications.filter(climate_risk_classification__in=[ClimateCreditApplication.CLIMATE_HIGH, ClimateCreditApplication.CLIMATE_SEVERE]).distinct()[:8]
    warning_alerts = applications.filter(early_warning_flag=True).distinct()[:10]
...
def apply_loan(request):
    ...
    if request.method == "POST":
        ...
        if not location_city or latitude == 0 or longitude == 0:
            return render(
                request,
                "apply_loan.html",
                {
                    "states": states,
                    "location_lookup": json.dumps(location_lookup),
                    "error": "Please search and select a location from the suggestions.",
                },
            )
        ...
        # Check for duplicate: prevent multiple applications with same borrower_id in same state/city
        existing = ClimateCreditApplication.objects.filter(
            user=request.user,
            borrower_id=borrower_id,
            location_state=location_state,
            location_district=location_city,
        ).first()
        ...
        if existing:
            ...
            return render(
                ...
                "apply_loan.html",
                {
                    "states": states,
                    "location_lookup": json.dumps(location_lookup),
                    "error": f"An application with this borrower already exists for {location_city}, {location_state}. Application ID: {existing.id}",
                    "success": False,
                },
            )
...
@require_http_methods(["POST"])
def fetch_location_api(request):
    """
    API endpoint to fetch coordinates from Nominatim (OpenStreetMap) for any location.
    Request: { "place": "Bangalore", "state": "Karnataka" }
    Response: { "lat": 12.9716, "lon": 77.5946, "place": "Bangalore" }
    """
    try:
        payload = json.loads(request.body.decode("utf-8"))
        place = str(payload.get("place", "")).strip()
    ...
    # Construct search query with state if provided
    search_query = f"{place} {state}" if state else place
    lat, lon = get_location_from_nominatim(search_query)
    ...
    if lat and lon:
        ...
        return JsonResponse({"error": f"Location '{place}' not found", "found": False}, status=404)
